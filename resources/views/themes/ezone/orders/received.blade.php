					...
					<div class="row">
						...
						<div class="col-xl-3 col-lg-4">
							...
							<address>
								...
								<br> {{ \General::datetime_format($order->order_date) }}
							...
							<tbody>
								@forelse ($order->orderItems as $item)
									<tr>
										...
										<td>{{ $item->qty }}</td>
										<td>{{ \General::price_format($item->base_price) }}</td>
										<td>{{ \General::price_format($item->sub_total) }}</td>
									</tr>
								@empty
									<tr>
					...
					<div class="row">
						<div class="col-md-5 ml-auto">
							<div class="cart-page-total">
								<ul>
									<li> Subtotal
										<span>{{ \General::price_format($order->base_total_price) }}</span>
									</li>
									<li>Tax (10%)
										<span>{{ \General::price_format($order->tax_amount) }}</span>
									</li>
									<li>Shipping Cost
										<span>{{ \General::price_format($order->shipping_cost) }}</span>
									</li>
									<li>Total
										<span>{{ \General::price_format($order->grand_total) }}</span>
									</li>
								</ul>
								<!-- @if (!$order->isPaid())  {{ $order->payment_url }} -->
